 ...
 * 3. Emits src/generated/fixtureRegistry.tsx (fixture id → lazy route) and
 *    src/generated/fixture-map.json (metadata for the runner). Named colors
 *    come from src/generated/theme.css, which `rjui build` itself writes and
 *    src/index.css imports — the same one-line wiring rjui prescribes to a
 *    consumer, so the host measures the product path.
 *
...
// ------------------------------------------------------------------ theme css
// rjui build extracts hex colors into Layouts/Resources/colors.json and emits
// `src/generated/theme.css` — an `@theme` block plus the utilities that have no
// Tailwind-core backing. That file IS the product path: rjui tells consumers to
// @import it from their globals.css, and `src/index.css` does the same here.
//
// The host used to synthesize its own plain-CSS shim (`.bg-<key> { … }`)
// instead. It resolved the base utilities and nothing else, so two whole
// classes of emit measured inert against their controls for a reason that
// lived in the measuring apparatus: `var(--color-<key>)` (nothing declares the
// custom property) and every variant form (`disabled:bg-<key>` — a plain rule
// carries no variants). Both were adjudicated onto cross_effect.json as web
// idioms before the shim was understood to be the cause.
const themeCssPath = path.join(generatedDir, 'theme.css');
if (!fs.existsSync(themeCssPath)) {
  throw new Error(
    `rjui build did not emit ${path.relative(hostDir, themeCssPath)}. The host resolves ` +
      'named colors through that file (src/index.css imports it); without it every ' +
      'palette-name emit silently renders as the inherited default.'
  );
}
const staleShim = path.join(generatedDir, 'conformance-colors.css');
if (fs.existsSync(staleShim)) fs.rmSync(staleShim);

console.log('[generate] wrote fixtureRegistry.tsx, fixture-map.json; theme.css from rjui build');
