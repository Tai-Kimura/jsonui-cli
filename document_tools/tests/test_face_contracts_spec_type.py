"""`face_contracts_spec` is accepted, and an unknown type is refused as one.
...
`{"type": "face_contract"}` was told `structure` was missing, `displayName`
...
def _face_spec(**over):
    spec = {
        "type": FACE_CONTRACTS_SPEC,
        "version": "1.0",
        "metadata": {"name": "user", "description": "units this face owns"},
        "unitContracts": [
...
class FaceContractsSpecIsAccepted(unittest.TestCase):
    def test_a_well_formed_face_spec_validates_clean(self):
        self.assertEqual([], [e.message for e in _errors(_face_spec())])

    def test_the_face_name_is_not_held_to_the_screen_rules(self):
        """A face name is not a class name.

        `metadata.name` here identifies the face, so the PascalCase rule and
        the `displayName` requirement — both correct for a screen — would
        report a correctly written face spec as malformed.
        """
        msgs = [e.message for e in _errors(_face_spec())]
...
class AnUnknownTypeIsRefusedAsATypeError(unittest.TestCase):
    def test_a_misspelled_face_type_reports_the_type_and_nothing_else(self):
        errs = _errors(_face_spec(type="face_contract"))
        self.assertEqual(1, len(errs), [e.message for e in errs])
        self.assertEqual("type", errs[0].path)
        self.assertIn("face_contract", errs[0].message)
    ...
    def test_it_is_not_reported_as_a_malformed_screen_spec(self):
        ...
        — as the problem.
        """
        msgs = " | ".join(e.message for e in _errors(_face_spec(type="face_contract")))
    ...
    def test_the_message_names_every_type_that_would_have_worked(self):
        msg = _errors(_face_spec(type="nonsense"))[0].message
...
class AFaceSpecCannotHoldScreenOwnedSections(unittest.TestCase):
    def test_each_forbidden_section_is_refused_by_name(self):
        for section in SpecValidator._FACE_SPEC_FORBIDDEN:
            with self.subTest(section=section):
                paths = [e.path for e in _errors(_face_spec(**{section: ["x"]}))]
    ...
    def test_branch_contracts_is_refused(self):
        ...
        differs: nothing yet decides which face owns a screenless branch, and
        writable-but-unread is the failure this type exists to remove."""
        paths = [e.path for e in _errors(_face_spec(branchContracts={"a": {}}))]
    ...
    def test_an_empty_forbidden_section_is_left_alone(self):
        ...
        self.assertEqual([], [e.message for e in _errors(_face_spec(structure=[]))])
