...
CLOSURE = {"Dashboard": {"CalendarWidget"}}
...
class EachSourceHasItsOwnControl(unittest.TestCase):
    def test_a_view_model_is_owned_by_its_screen(self):
        self.assertEqual(
            ["AccountSettings"],
            ownership.owner_screens("AccountSettingsViewModel", SCREENS, CLOSURE))

    def test_a_named_repository_is_owned_by_the_screen_that_names_it(self):
        self.assertEqual(
            ["Listing"],
            ownership.owner_screens("ListingRepository", SCREENS, CLOSURE))

    def test_a_named_use_case_is_owned_by_the_screen_that_names_it(self):
        self.assertEqual(
            ["Checkout"],
            ownership.owner_screens("ConfirmUseCase", SCREENS, CLOSURE))

    def test_a_component_is_owned_through_the_layout_closure(self):
        self.assertEqual(
            ["Dashboard"],
            ownership.owner_screens("CalendarWidget", SCREENS, CLOSURE))
    ...
    def test_a_shared_utility_is_owned_by_no_screen(self):
        ...
        self.assertEqual(
            [], ownership.owner_screens("SharedHttpClient", SCREENS, CLOSURE))
    ...
    def test_the_four_controls_do_not_share_an_answer(self):
        ...
        answers = [
            ownership.owner_screens(t, SCREENS, CLOSURE)
...
class TheCardinalityDecidesTheDeclarationSite(unittest.TestCase):
    ...
    def test_a_symbol_nothing_defines_is_unresolved_not_app_owned(self):
        ...
        permission."""
        kind, _ = ownership.classify("SharedHtpClient", SCREENS, CLOSURE, KNOWN)
...
class AMissingInputIsNotAPermission(unittest.TestCase):
    def test_without_the_closure_a_zero_is_undetermined(self):
        """The failure this guards against: a caller that has not computed the
        layout closure sees component-owned targets as owned by nobody."""
        kind, _ = ownership.classify(
