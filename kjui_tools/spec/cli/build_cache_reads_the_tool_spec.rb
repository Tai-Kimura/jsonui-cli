...
RSpec.describe 'the kjui build cache reads the tool and the components' do
  def tool_root
  ...
  def extensions
    Dir.glob(File.join(@dir, 'kjui_tools', 'lib', '**', 'extensions'))
       .find { |d| File.directory?(File.join(d, 'attribute_definitions')) }
  end

  before(:all) do
    @dir = Dir.mktmpdir('kjui_cache_inputs')
    ...
    File.write(File.join(@dir, 'kjui.config.json'), JSON.pretty_generate(
      'mode' => 'compose', 'project_name' => 'CacheInputs',
    ...
    run('g', 'converter', 'Box', '--attr', 'title:String', '--container', '--force')
    ...
    builtin = File.join(@dir, 'kjui_tools', 'lib', 'compose', 'components', 'text_component.rb')
