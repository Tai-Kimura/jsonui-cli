...
# The warning `kjui g converter --attr key:Type` prints for a type
...
#
# End to end: the Kotlin the warning names is read back from the composable
# written. The Swift it names for `Row!!` is sjui's rule (sjui's arm reads it
# back from the Swift written); kjui writes no Swift, so here it is held to
# that rule as a literal. The tool is COPIED (links dereferenced).
RSpec.describe 'the warning for an attribute type outside the vocabulary, through kjui g converter' do
  def tool_root
    File.expand_path('../..', __dir__)
  end

  # type => [what the composable declares for it, what sjui's Swift scaffold
  ...
  before(:all) do
    @dir = Dir.mktmpdir('kjui_attr_warning')
    ...
    File.write(File.join(@dir, 'kjui.config.json'), JSON.pretty_generate(
      'mode' => 'compose', 'project_name' => 'WarnProbe',
    ...
    self.class.declares.each_key.with_index do |type, i|
      @runs[[type, :new]] = kjui.call("New#{i}", type, ['--force'])
      kjui.call("Kept#{i}", 'Int', ['--force'])
      @runs[[type, :kept]] = kjui.call("Kept#{i}", type, ['--skip-existing'])
      kjui.call("Forced#{i}", 'Int', ['--force'])
      @runs[[type, :forced]] = kjui.call("Forced#{i}", type, ['--force'])
    end
  end
  ...
  # What the composable declares for `r`.
  def declared(name)
    kotlin = File.read(File.join(@dir, 'app', 'src', 'main', 'kotlin', 'com', 'example', 'app', 'extensions', "#{name}.kt"))
    kotlin[/^\s*r: (.+?) = /, 1]
  end

  declares.each do |type, (kotlin, swift)|
    context type do
      %i[new forced].each do |run|
        it "#{run}: names #{kotlin} in Kotlin — what the composable declares — and #{swift} in Swift" do
          said = warning(type, run)
          expect(declared(component(type, run))).to eq(kotlin)
          expect(said[/it is scaffolded as (\S+) in Swift, (\S+) in Kotlin/, 2]).to eq(kotlin), said
      ...
      it 'kept (--skip-existing): says the scaffold was kept, not what it is scaffolded as' do
        ...
        # Nothing was written: the kept composable still declares its first type.
        expect(declared(component(type, :kept))).to eq('Int')
        expect(said).not_to include('is scaffolded as')
        expect(said).to include('the existing scaffold was kept')
        expect(said[/a new scaffold would declare (\S+) in Swift, (\S+) in Kotlin/, 2]).to eq(kotlin), said
