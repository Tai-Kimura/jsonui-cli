...
# What `kjui g view / partial / collection / adapter` print about each file,
# held to what happened on disk (a snapshot before and after each run), on a
...
# "Generated Collection Cell view:" with five files it had kept; `g adapter`
# "Successfully generated adapter" after keeping it, and patched the
# registry with an unchecked sub!. Ticket kjui-g-view-reports-what-it-did-not-do.
RSpec.describe 'what kjui g view / partial / collection / adapter say they did, against the disk' do
  def tool_root
  ...
  def self.commands
    [
      ['view', 'compose', %w[view home]],
      ['partial', 'compose', %w[partial header]],
      ['collection', 'compose', %w[collection item_cell]],
      ['adapter', 'compose', %w[adapter Card]]
    ]
  end
  ...
  def project(dir, mode)
    ...
    File.write(File.join(dir, 'kjui.config.json'), JSON.pretty_generate(
      'mode' => mode, 'project_name' => 'P', 'source_directory' => 'app/src/main',
  ...
  def snapshot(dir)
    Dir.glob(File.join(dir, '**', '*'), File::FNM_DOTMATCH).select { |f| File.file?(f) }
       .reject { |f| f.include?('/kjui_tools/') }
       .to_h { |f| [File.realpath(f), File.mtime(f)] }
  end

  def run_twice(mode, args)
    dir = Dir.mktmpdir('kjui_g_report')
    ...
    aggregate_failures do
      @runs.each do |label, runs|
        runs.each do |run, said, disk, _|
          ...
          expect(said).not_to match(/Successfully generated|^Generated |Files? created/),
                              "#{label} (#{run}): #{said}"
        end
      end
    end
