...
RSpec.describe 'the refusal reads the drawn tree, through kjui build' do
  def tool_root
  ...
  def gen(name, flag)
    run('g', 'converter', name, '--attr', 'title:String', flag, '--force')
  end
  ...
  before(:all) do
    @dir = Dir.mktmpdir('kjui_drawn_tree')
    ...
    File.write(File.join(@dir, 'kjui.config.json'), JSON.pretty_generate(
      'mode' => 'compose', 'project_name' => 'DrawnTree',
