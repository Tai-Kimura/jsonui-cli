...
RSpec.describe 'a leaf declared in its extension definition' do
  ...
  def defs_path
    %w[kjui_tools lib compose components extensions attribute_definitions]
  end
  ...
  def validator
    KjuiTools::Core::AttributeValidator.new(mode)
  end
  ...
  def refusals(layout)
    JsonUIShared::LayoutValidator.validate_layout(
      ...
      extension_definitions: KjuiTools::Core::AttributeValidator.extension_definitions(mode)
  ...
  describe 'the attribute validator' do
    ...
    it 'reads the project definitions from where the build reads them' do
      scaffold('Leaf', false)
      expect(KjuiTools::Core::AttributeValidator.extension_definitions(mode)['Leaf']).to include('_children' => 'none')
    end
  end
end
