  ...
  module Generated
    ...
    module LabelAttributes
      ...
      ATTRS = [
        ...
        # Enable URL/phone detection. Auto-links BARE http(s) URLs and phone numbers found in the text. It does not parse markdown, and relative paths are deliberately never linked: a path like "/guides/x" only means something to a web router, so linkifying it would produce a control that is dead on iOS and Android. Express a cross-document reference with partialAttributes 'range' + 'onclick', where the handler performs the navigation itself — including scrolling to the target position, which an href cannot express on mobile anyway. See partialAttributes for the measured per-platform support before relying on it.
        ...
        # Partial text styling: apply font/size/color/underline/shadow to a substring selected by 'range' (a [start, end] pair, a text pattern, or a binding), and optionally make it tappable with 'onclick'. This is the supported way to get emphasis or a link inside a Label, since 'text' is plain text and markdown is not interpreted. PLATFORM SUPPORT IS NOT EQUAL, measured 2026-07-27: iOS and Android hand the partials to their runtime, which sees the resolved string, so pattern and binding ranges work and localized text is fine. WEB renders partials at BUILD time by slicing the literal 'text', so only [start, end] works there, and combining it with a localized or bound 'text' slices the raw key instead of the resolved string. Web now warns on both rather than dropping them silently. Until web renders partials at runtime, a cross-reference inside localized prose is not expressible on web — that is a known gap, not a usage error.
