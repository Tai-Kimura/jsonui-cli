...
# What `rjui g converter` prints about each file, held to what happened on
...
RSpec.describe 'what rjui g converter says it wrote, against the disk' do
  def tool_root
  ...
  def run_tool(dir, tool, flags, stdin)
    said, status = Open3.capture2e('ruby', File.join(tool, 'bin', 'rjui'), 'g', 'converter', 'Probe',
  ...
  def snapshot(dir)
    Dir.glob(File.join(dir, '**', '*')).select { |f| File.file?(f) }
       .reject { |f| f.include?('/rjui_tools/') && !f.include?('/extensions/') }
  ...
  describe 'the scaffold' do
    ...
    def scaffold_files
      [['probe_converter.rb', 'converter file', 'converter'], ['Probe.tsx', 'component file', 'component']]
    end
