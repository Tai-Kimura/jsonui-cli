...
module SjuiTools
  module Core
    ...
    # Used by both UIKit and SwiftUI converters
    class AttributeValidator
      ...
      def print_infos
        @infos.each do |info|
          puts "\e[36mℹ️  [SJUI Info] #{info}\e[0m"
        end
      end
      ...
      def load_definitions
        ...
        base_definitions = if File.exist?(definitions_path)
          ...
          puts "\e[31m[SJUI Error] attribute_definitions.json not found at #{definitions_path}\e[0m"
          {}
        end
        ...
        extension_paths.each do |ext_dir|
          ...
          Dir.glob(File.join(ext_dir, '*.json')).each do |file|
            begin
              ...
              puts "\e[33m[SJUI Warning] Failed to parse extension definition #{file}: #{e.message}\e[0m"
            end
          end
        end
      ...
      def map_type_to_definition(type)
        ...
        when 'Image', 'ImageView'
          'Image'
        when 'NetworkImage', 'NetworkImageView', 'CircleImage', 'CircleImageView'
          'NetworkImage'
        when 'SelectBox', 'Spinner', 'DatePicker'
          'SelectBox'
        when 'Toggle', 'Switch'
          'Toggle'
        when 'Segment', 'SegmentedControl'
          'Segment'
        when 'Slider', 'SeekBar'
          'Slider'
        ...
        when 'ScrollView', 'Scroll'
          'ScrollView'
        when 'Collection', 'CollectionView', 'RecyclerView', 'Table', 'TableView'
          'Collection'
        when 'Radio', 'RadioButton'
          'Radio'
        when 'CheckBox', 'Check'
          # Map both CheckBox and Check to their respective definitions
      ...
      # - widthWeight can substitute for width (UIKit mode)
      # - heightWeight can substitute for height (UIKit mode)
      ...
      def skip_dimension_required?(attr_name, component, parent_orientation)
        ...
        # Check for specific dimension weight (UIKit mode)
      ...
      # Load sjui.config.json if it exists
      # @return [Hash, nil] Config hash or nil
      def load_sjui_config
