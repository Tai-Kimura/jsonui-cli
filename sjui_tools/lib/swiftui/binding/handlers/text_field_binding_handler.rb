      ...
      class TextFieldBindingHandler < ViewBindingHandler
        ...
        def get_text_binding(component)
          ...
          if is_binding?(text_value)
            ...
            ".constant(#{JsonUIShared::StringLiterals.swift(text_value || '')})"
          end
        end

        # Check if this should be a SecureField
        # Priority: explicit secure attribute > input type
        def is_secure_field?(component)
          if component.key?('secure')
            return component['secure'] == true || component['secure'] == 'true'
          end
