    ...
    module Setup
      class SwiftUISetup < ::SjuiTools::Core::PbxprojManager
        ...
        def create_string_manager_file(resource_manager_path)
          ...
          # Create ColorManager.swift in ResourceManager directory
          create_color_manager_file(resource_manager_path)
        end

        def create_color_manager_file(resource_manager_path)
          color_manager_path = File.join(resource_manager_path, 'ColorManager.swift')
          unless File.exist?(color_manager_path)
            # Create empty ColorManager.swift file
            File.write(color_manager_path, "// ColorManager.swift\n// Implementation will be added separately\n")
            puts "Created file: ColorManager.swift"

            # Add to Xcode project
            add_file_to_project(color_manager_path)
          else
            puts "File already exists: ColorManager.swift"
          end
        end
      end
    end
