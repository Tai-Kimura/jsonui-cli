        ...
        def wrap_child_for_alignment(child_lines, orientation, needs_wrapper, wrapper_alignment)
          if needs_wrapper && wrapper_alignment
            if orientation == 'horizontal'
              ...
              add_line "VStack {"
              indent do
                # Child converter was created with @indent_level + 1, add directly to preserve nested structure
                child_lines.each { |line| @generated_code << line unless line.strip.empty? }
              end
