  ...
  module UIKit
    class BindingFileManager
      ...
      def setup_binding_file_info(file_name)
        ...
        # Check for ViewController in top-level directory
        if File.exist?("#{@view_path}/#{@base_name}/#{@base_name}ViewController.swift")
          @super_binding = "BaseBinding"
        else
          # Search subdirectories for ViewController (e.g., Mypage/ChangeEmailSheet/)
          vc_pattern = File.join(@view_path, '**', @base_name, "#{@base_name}ViewController.swift")
          if Dir.glob(vc_pattern).any?
            @super_binding = "BaseBinding"
          end
        end
