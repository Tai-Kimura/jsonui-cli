    ...
    class ImageBindingHandler < ViewBindingHandler
      def handle_specific_binding(view_name, key, value)
        ...
        when "srcName"
          ...
          elsif optional?(value)
            # Optional variable - use nil coalescing and force unwrap
            @binding_content << "        #{view_name}?.image = !(#{value} ?? \"\").isEmpty ? UIImage(named: #{value}!) : nil\n"
          else
            # Non-optional variable - no need for nil coalescing or force unwrap
            @binding_content << "        #{view_name}?.image = !#{value}.isEmpty ? UIImage(named: #{value}) : nil\n"
          end
        when "highlightSrcName"
          if value.end_with?("!!")
            ...
            # Optional variable - check for nil and force unwrap
            @binding_content << "        #{view_name}?.highlightedImage = #{value} != nil ? UIImage(named: #{value}!) : nil\n"
          else
            # Non-optional variable - no need for nil check or force unwrap
            @binding_content << "        #{view_name}?.highlightedImage = !#{value}.isEmpty ? UIImage(named: #{value}) : nil\n"
          end
        when "src"
