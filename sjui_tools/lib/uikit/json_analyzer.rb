...
module SjuiTools
  module UIKit
    class JsonAnalyzer
      attr_reader :binding_content, :data_sets, :binding_processes_group, :including_files, :reset_constraint_views, :weak_vars_content, :invalidate_methods_content, :partial_bindings, :view_variables, :collection_data_bindings
      ...
      def initialize(import_module_manager, ui_control_event_manager, layout_path, style_path, super_binding, view_type_set)
        ...
        @validator = Core::AttributeValidator.new(:uikit)
        @collection_data_bindings = {}  # Track CollectionView items bindings: data_name => [view_name, ...]
      end

      # Class-level validation toggle
      ...
      def analyze_json(file_name, loaded_json, element_path = [], parent_orientation = nil)
        ...
        if json["include"] && !json["type"]
          ...
          # Use 'id' as binding_id if present, otherwise use explicit 'binding_id'
          binding_id = json["id"] || json["binding_id"]
          process_include_element(file_name, include_name, json, shared_data_bindings, binding_id, data_bindings, parent_orientation)
        ...
        # Validate component attributes AFTER style merge
        # スタイルファイルのtypeは削除されているため、コンポーネントの元のtypeで検証される
        if @@validation_enabled
          # element_path has format "child[0] (Type)" but we want just "child[0]" for hierarchy
          # since the validator already shows the type separately
          hierarchy = element_path.empty? ? nil : element_path.map { |p| p.gsub(/ \([^)]+\)$/, '') }.join('.')
          @validator.validate(json, nil, parent_orientation, file_name: file_name, view_id: json['id'], hierarchy: hierarchy)
        ...
        json.each do |key,value|
          ...
          when "data"
            ...
            # Normalize types using TypeConverter (mode: uikit)
            if @current_partial_depth == 0
              normalized_data = Core::TypeConverter.normalize_data_properties(value, 'uikit')
              @data_sets += normalized_data
            end
          when "type"
          ...
          when "child"
            # childが単一オブジェクトの場合と配列の場合の両方に対応
            children = value.is_a?(Array) ? value : [value]
            # Get current component's orientation to pass to children
            current_orientation = json["orientation"] || parent_orientation
            children.each_with_index do |child_json, index|
              child_type = child_json["type"] || "element"
              child_path = element_path + ["child[#{index}] (#{child_type})"]
              analyze_json(file_name, child_json, child_path, current_orientation)
            end
          when "include"
            # Use 'id' as binding_id if present, otherwise use explicit 'binding_id'
            binding_id = json["id"] || json["binding_id"]
            ...
            # Get current component's orientation to pass to include
            current_orientation = json["orientation"] || parent_orientation
            process_include_element(file_name, value, json, shared_data_bindings, binding_id, data_bindings, current_orientation)
            # Track this as a partial binding only if we're at the top level
          ...
          # TextField/TextView event handlers (closure-based with @{})
          when "onBeginEditing", "onEndEditing", "onChangeSelection"
            next unless value.is_a?(String) && value.start_with?("@{")
            if %w[TextField EditText Input].include?(current_view["type"])
              @ui_control_event_manager.add_text_field_event(current_view["name"], key, value, original_id: current_view["original_id"]) if @current_partial_depth == 0
            elsif current_view["type"] == "TextView"
              @ui_control_event_manager.add_text_view_event(current_view["name"], key, value, original_id: current_view["original_id"]) if @current_partial_depth == 0
            end
          when "onTextChange"
            # @{} format: use closure-based handler
            # plain string: use legacy selector format (handled by SJUITextField/SJUITextView.createFromJSON)
            ...
            # If not @{} format, do nothing here - it will be handled by createFromJSON as selector
          when "onDeleteBackward", "onShouldReturn", "onShouldChangeCharacters", "onShouldClear", "onShouldBeginEditing", "onShouldEndEditing"
            # TextField-specific event handlers (closure-based with @{})
            next unless value.is_a?(String) && value.start_with?("@{")
            if %w[TextField EditText Input].include?(current_view["type"])
              @ui_control_event_manager.add_text_field_event(current_view["name"], key, value, original_id: current_view["original_id"]) if @current_partial_depth == 0
            end
          when "onShouldChangeText"
            # TextView-specific event handler (closure-based with @{})
            next unless value.is_a?(String) && value.start_with?("@{")
            if current_view["type"] == "TextView"
              @ui_control_event_manager.add_text_view_event(current_view["name"], key, value, original_id: current_view["original_id"]) if @current_partial_depth == 0
            end
          else
      ...
      def analyze_binding_process(binding_process)
        ...
        # CollectionViewのitemsバインディングは特別扱い
        # invalidateではなくdidSetで処理するため、ここでは追跡のみ行う
        if view["type"] == "Collection" && key == "items"
          track_collection_data_binding(view_name, value)
          return
        end

        # ハンドラーを取得
        handler = ViewBindingHandlerFactory.create_handler(view["type"], @binding_content, @reset_text_views, @reset_constraint_views, @data_sets)
      ...
      # CollectionViewのitemsバインディングを追跡
      # didSet生成時に使用する
      def track_collection_data_binding(view_name, data_name)
        @collection_data_bindings[data_name] ||= []
        @collection_data_bindings[data_name] << view_name unless @collection_data_bindings[data_name].include?(view_name)
      end
      ...
      def has_binding_data(json)
        ...
        # Event handlers (onClick, onLongPress, etc.) are NOT counted as data bindings
        # because they are handled in bindView(), not invalidateAll()
        event_handler_keys = %w[onClick onLongPress onPan onPinch]
        ...
        if json.is_a?(Hash)
          ...
          json.each do |key, value|
            # Skip event handler keys - they don't generate invalidateAll code
            next if event_handler_keys.include?(key)
      ...
      def process_id_element(json, value, current_view)
        ...
        current_view["original_id"] = value
      ...
      def process_include_element(file_name, value, json, shared_data_bindings = nil, binding_id = nil, data_bindings = nil, parent_orientation = nil)
        ...
        File.open(file_path, "r") do |file|
          ...
          analyze_json(file_name, included_json, [], parent_orientation)
      ...
      def process_binding_element(json, key, value, current_view, file_name = nil, element_path = [])
        if value.is_a?(String) && value.start_with?("@{")
          ...
          group_names.each do |group_name|
            ...
            group = [] if group.nil?
            # Also include partialAttributes if present (for range bindings etc.)
            unless json["partialAttributes"].nil?
              # Check if partialAttributes already added to avoid duplication
              already_added = group.any? { |g| g[:view] == current_view && g[:key] == "partialAttributes" }
              group << {"view": current_view, "key": "partialAttributes", "value": json["partialAttributes"]} unless already_added
            end
        ...
        # Handle partialAttributes with bindings inside (e.g., onClick: "@{handler}")
        # This handles the case when partialAttributes has bindings but no other binding attributes exist
        if key == "partialAttributes" && value.is_a?(Array)
          return if @current_partial_depth > 0

          has_binding = value.any? do |pa|
            next false unless pa.is_a?(Hash)
            pa.values.any? do |v|
              if v.is_a?(String)
                v.start_with?("@{")
              elsif v.is_a?(Array)
                v.any? { |item| item.is_a?(String) && item.start_with?("@{") }
              else
                false
              end
            end
          ...
          if has_binding
            ...
            group_names.each do |group_name|
              group = @binding_processes_group[group_name]
              group = [] if group.nil?
              # Check if already added by other binding processing
              already_added = group.any? { |g| g[:view] == current_view && g[:key] == "partialAttributes" }
              unless already_added
                group << {"view": current_view, "key": "partialAttributes", "value": value}
                @binding_processes_group[group_name] = group
              end
            end
          end
