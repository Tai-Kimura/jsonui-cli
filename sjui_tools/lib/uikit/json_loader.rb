  ...
  module UIKit
    class JsonLoader
      ...
      def initialize(project_root_path = nil, project_file_path = nil)
        ...
        @string_manager.cache_strings_files(string_files) if string_files.any?
      end
      ...
      def start_analyze
        ...
        Dir.glob("#{@layout_path}/**/*.json") do |file|
          ...
          # Check JSON-level mode filter: skip files with "mode": "swiftui"
          begin
            json_content = JSON.parse(File.read(file))
            file_mode = json_content['mode']
            if file_mode && file_mode.downcase == 'swiftui'
              Core::Logger.debug "Skip: #{file} (mode: swiftui)"
              next
            end
          rescue JSON::ParserError
            # If JSON is invalid, let it fail later in the normal processing
          end
          ...
          File.open(file, "r") do |f|
            ...
            # Check for data property / view ID conflicts
            check_id_data_conflicts(file, @json_analyzer.data_sets, @json_analyzer.view_variables)
          end
                    ...
                    if data_with_default.is_a?(Hash) && data_with_default.has_key?("defaultValue")
                      ...
                      if data_with_default["class"] == "String"
                        ...
                        if value_str == "''" || value_str.empty?
                          # Empty string - no localization needed
                          default_value = '""'
      ...
      def generate_bind_view_with_partials
        ...
        if base_content.empty?
          ...
          # Insert view assignments and partial binding calls into existing bindView
          ...
          if insert_pos
            ...
            unless @json_analyzer.view_variables.empty?
              insert_content << "        \n"
              insert_content << "        // Assign views from ViewHolder\n"
              @json_analyzer.view_variables.each do |view_var|
                insert_content << "        #{view_var[:variable_name]} = getView(\"#{view_var[:original_id]}\", bindingId: bindingId)\n"
              end
              insert_content << "        \n"
            end
            ...
            base_content.insert(insert_pos, insert_content)
          end
          base_content
        end
      end
      ...
      def generate_binding_file(binding_info)
        # Check if Combine import is needed (PassthroughSubject, CurrentValueSubject in data)
        needs_combine = @json_analyzer.data_sets.any? { |d|
          d.is_a?(Hash) && (d['class'].to_s.include?('PassthroughSubject') || d['class'].to_s.include?('CurrentValueSubject'))
        }
        @import_modules_extra = needs_combine ? "import Combine\n" : ""

        # インポート文の生成
        import_content = @import_module_manager.generate_import_statements + @import_modules_extra
        
        # bindingクラスのヘッダー生成
        header_content = generate_class_header(binding_info)

        # invalidateメソッドの生成（先に呼び出してcollection_data_bindingsを設定）
        @json_analyzer.generate_invalidate_methods
        invalidate_content = @json_analyzer.invalidate_methods_content

        # data変数の生成（collection_data_bindingsが設定された後に呼ぶ）
      ...
      def check_id_data_conflicts(file, data_sets, view_variables)
        data_names = data_sets.map { |d| d.is_a?(Hash) ? d['name'] : d }.compact
        view_var_names = view_variables.map { |v| v[:variable_name] }
        conflicts = data_names & view_var_names
      ...
      def generate_data_variables(binding_info)
        ...
        @json_analyzer.data_sets.each do |data|
          ...
          if data.is_a?(String)
            ...
            # Check if this data is bound to a CollectionView's items
            has_collection_binding = check_data_bound_to_collection(data_name)

            if data["defaultValue"].nil?
              # Don't add ? if the type already ends with ? (e.g., function types)
              type_suffix = data["class"].to_s.end_with?('?') ? '' : '?'
              if has_child_bindings || has_collection_binding
                content << "    #{modifier} #{data_name}: #{data["class"]}#{type_suffix} {\n"
                content << "        didSet {\n"
                content << generate_didset_content_for_data(data_name)
                content << generate_didset_content_for_collection(data_name)
                content << "        }\n"
                content << "    }\n"
              else
                content << "    #{modifier} #{data_name}: #{data["class"]}#{type_suffix}\n"
      ...
      # Check if a data property is bound to a CollectionView's items
      def check_data_bound_to_collection(data_name)
        @json_analyzer.collection_data_bindings.key?(data_name)
      end

      # Generate didSet content for CollectionView data bindings
      def generate_didset_content_for_collection(data_name)
        content = String.new
        view_names = @json_analyzer.collection_data_bindings[data_name]
        return content unless view_names

        view_names.each do |view_name|
          content << "            #{view_name}?.reloadWithDataSource(#{data_name})\n"
        end
        content
      end
