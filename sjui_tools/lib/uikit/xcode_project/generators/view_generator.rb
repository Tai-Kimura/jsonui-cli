  ...
  module UIKit
    module XcodeProject
      module Generators
        class ViewGenerator < ::SjuiTools::Core::PbxprojManager
          def initialize(name, options = {})
            ...
            @viewmodel_path = File.join(source_path, @config['viewmodel_directory'] || 'ViewModel')
            @xcode_manager = SjuiTools::Core::XcodeProjectManager.new(@project_file_path)

            # Build command string for comments
      ...
      class #{camel_name}ViewController: BaseViewController {
          ...
          let viewModel: #{camel_name}ViewModel

          init(viewModel: #{camel_name}ViewModel = #{camel_name}ViewModel()) {
              self.viewModel = viewModel
              super.init(nibName: nil, bundle: nil)
          }

          required init?(coder: NSCoder) {
              self.viewModel = #{camel_name}ViewModel()
      ...
      #{marker_footer}
        ...
        def generate_viewmodel_content(camel_name, snake_name)
          <<~SWIFT
      //
      //  #{camel_name}ViewModel.swift
