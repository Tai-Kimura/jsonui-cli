...
# A leaf (`sjui g converter <Name> --no-container`) turned back into a
...
# drew the component without them — rc 0, not a word. The kept adapter still
# said `acceptsChildren: Bool { false }`. Ticket
...
# views/extensions.
RSpec.describe 'a leaf turned back into a container, through sjui g converter and sjui build' do
  def tool_root
  ...
  def self.reruns
    {
      'Kept' => [['--container', '--skip-existing'], '', %w[kept_converter.rb KeptAdapter.swift]],
      'Closed' => [['--container'], '', %w[closed_converter.rb ClosedAdapter.swift]],
      # "n" to the converter, "y" to the Swift file and the adapter
      'Half' => [['--container'], "n\ny\ny\n", %w[half_converter.rb]],
      # "y" to the converter; the Swift file and the adapter read EOF, "n"
      'Viewed' => [['--container'], "y\n", %w[ViewedAdapter.swift]],
  ...
  before(:all) do
    @dir = Dir.mktmpdir('sjui_leaf_turned')
    ...
    name = 'TurnProbe'
    # adapter_directory: the adapter is one of the files a leaf keeps.
    ...
    sjui = lambda do |component, args, stdin|
    ...
    self.class.reruns.each do |component, (args, stdin, _)|
      @first[component] = sjui.call(component, ['--no-container', '--force'], '')
      @rerun[component] = sjui.call(component, args, stdin)
  ...
  def definition(component)
    path = File.join(@dir, 'sjui_tools', 'lib', 'swiftui', 'views', 'extensions', 'attribute_definitions',
                     "#{component}.json")
  ...
  def drawn?(component)
    Dir.glob(File.join(@dir, '**', '*GeneratedView.swift'))
