...
RSpec.describe 'the refusal reads the drawn tree, through sjui build' do
  def tool_root
  ...
  before(:all) do
    @dir = Dir.mktmpdir('sjui_drawn_tree')
    ...
    File.write(File.join(@dir, 'sjui.config.json'), JSON.pretty_generate(
      'mode' => 'swiftui', 'project_name' => 'DrawnTree', 'project_file_name' => 'DrawnTree',
      'source_directory' => 'DrawnTree', 'layouts_directory' => 'Layouts',
      ...
      'string_files' => ['DrawnTree/Localizable.strings'], 'use_network' => true
    ))
    FileUtils.mkdir_p(File.join(@dir, 'DrawnTree.xcodeproj'))
    @layouts = File.join(@dir, 'DrawnTree', 'Layouts')
  ...
  {
    'X1 an included screen layout' => %w[x1_home.json x1_part.json],
    'X2 an included partial' => %w[x2_home.json x2_part.json],
    'X3 a partial including a partial' => %w[x3_home.json x3_inner.json x3_outer.json],
    'X4 a variant including a partial' => %w[x4_home@regular.json x4_part.json],
    'X5 cellClasses without sections, inside a partial' => %w[x5_home.json x5_part.json],
    'X6 an include with an id' => %w[x6_home.json x6_part.json],
    'X7 a Collection cell layout' => %w[x7_cell.json]
  }.each do |row, files|
    it "refuses #{row}: the layout that draws it, and the file it sits in" do
      prefix = row[/X\d/].downcase
      expect(refused(@fresh).select { |f| f.start_with?(prefix) }).to eq(files.sort), @fresh
    end
  end

  it 'answers the same on a cached build: a component in an included partial made a leaf (C1)' do
    expect(@cached).not_to include('No files need updating'), @cached
    expect(refused(@cached)).to include('c1_home.json', 'c1_part.json'), @cached
  end
end
