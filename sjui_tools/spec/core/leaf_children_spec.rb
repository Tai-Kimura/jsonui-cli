...
# spec/cli/commands/leaf_children_build_spec.rb.
RSpec.describe 'a leaf declared in its extension definition' do
  ...
  def defs_path
    %w[sjui_tools lib swiftui views extensions attribute_definitions]
  end
  ...
  def validator
    SjuiTools::Core::AttributeValidator.new(mode)
  end
  ...
  def refusals(layout)
    JsonUIShared::LayoutValidator.validate_layout(
      ...
      extension_definitions: SjuiTools::Core::AttributeValidator.extension_definitions(mode)
  ...
  describe 'the attribute validator' do
    ...
    it 'reads the project definitions from where the build reads them' do
      scaffold('Leaf', false)
      expect(SjuiTools::Core::AttributeValidator.extension_definitions(mode)['Leaf']).to include('_children' => 'none')
    end
  end
end
