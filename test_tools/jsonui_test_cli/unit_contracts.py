...
def discover_unit_contracts(
...
) -> tuple[list[UnitCase], list[str]]:
    """``(declared cases, every spec scanned)``.
    ...
    for path in _spec_files(spec_path):
        ...
        cases.extend(_cases_of(spec, screen))
    return cases, scanned


def _cases_of(spec: dict, screen: str) -> list[UnitCase]:
    blocks = spec.get("unitContracts")
...
def check_unit_contracts(
    ...
    cases, scanned = discover_unit_contracts(project_root, spec_dir)
    roots = _test_roots(project_root, config)
    if project_platforms is None:
        project_platforms = sorted(roots)

    report = UnitContractReport(cases=cases, scanned_specs=scanned)
    for case in cases:
