...
The probe on each face: a view model sends A, then B 50 ms later, and
...
Each delay is longer than the face's fixed drain (web: ten setTimeout(0)
turns; iOS: 80 x 5 ms), so a settle that did not wait could not see it land.
...
// The view model: A, then B 50 ms later; fire-and-forget, like a screen load.
void fetch("https://x.test/a").then(() => order.push("a"));
setTimeout(() => void fetch("https://x.test/b").then(() => order.push("b")), 50);
...
def _run_web(tmp_path: Path, overrides: dict, runtime: str | None = None) -> dict:
    tc.tool("node")
    (tmp_path / "runtime.ts").write_text(runtime or bt.RUNTIME_TS, encoding="utf-8")
    (tmp_path / "probe.ts").write_text(_TS_PROBE % {"delay": DELAY_MS}, encoding="utf-8")
    run = subprocess.run(["node", "--experimental-strip-types", "probe.ts", json.dumps(overrides)],
                         cwd=tmp_path, capture_output=True, text=True, timeout=120)
    assert run.returncode == 0, run.stderr[:3000]
    return dict(line.split(" ", 1) for line in run.stdout.strip().splitlines())


@pytest.mark.parametrize("overrides, order, waits", [
    ({}, "a,b", False),
    ({"a": "slow"}, "b,a", True),
    ({"b": "slow"}, "a,b", True),
])
...
def _web_without_the_wait(runtime: str) -> str:
    ...
    return runtime.replace(wait, "    return;\n    await Promise.race([\n")


def test_web_control_a_settle_that_does_not_wait_reads_before_the_arrival(tmp_path):
    got = _run_web(tmp_path, {"a": "slow"}, _web_without_the_wait(bt.RUNTIME_TS))
...
def test_web_past_the_budget_settle_fails_by_name(tmp_path):
    ...
    assert out.startswith("THROWN settle: 1 delayed response(s) still pending after waiting "), out
...
for arg in CommandLine.arguments.dropFirst() { overrides[arg] = "slow" }
let dummy = NSObject()
...
def _build_swift(work: Path, runtime: str, main: str = _SWIFT_MAIN) -> Path:
    tc.tool("swiftc")
    work.mkdir(parents=True, exist_ok=True)
    (work / "shim.swift").write_text(_SWIFT_SHIM, encoding="utf-8")
    (work / "runtime.swift").write_text(runtime, encoding="utf-8")
    (work / "main.swift").write_text(main % {"delay": DELAY_MS}, encoding="utf-8")
    binary = work / "prog"
...
def _run_swift(binary: Path, *slow: str) -> dict:
    run = subprocess.run([str(binary), *slow], capture_output=True, text=True, timeout=300)
    assert run.returncode == 0, run.stderr[:3000]
    return dict(line.split(" ", 1) for line in run.stdout.strip().splitlines())
...
def _swift_without_the_wait(runtime: str) -> str:
    wait = "      while BranchDeliveries.shared.pending > 0 {\n"
    assert runtime.count(wait) == 1
    return runtime.replace(wait, "      return\n" + wait)
...
def test_ios_past_the_budget_settle_fails_by_name(swift_runtime, tmp_path):
    ...
    assert len(fails) == 1 and "delayed response(s) still pending after waiting" in fails[0], run.stdout
